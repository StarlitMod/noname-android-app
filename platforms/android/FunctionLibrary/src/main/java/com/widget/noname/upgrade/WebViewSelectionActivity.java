...
public class WebViewSelectionActivity extends Activity {
    ...
    private static final int NOT_INSTALL = R.string.common_not_install;

    private static final int UNSUPPORTED = R.string.common_unsupported;

    private static final int NOT_ENABLED = R.string.common_not_enabled;
    ...
    private void updateWebViewOption(LinearLayout webViewLayout, String packageName) {
        ...
        if (subtitleView != null) {
            ...
            if (systemWebViewPackageName != null && systemWebViewPackageName.equals(packageName)) {
                subtitleView.setText(subtitleView.getText() + "(" + getString(R.string.common_system_in_use) + ")");
            }
            if (selected_webview_package != null && selected_webview_package.equals(packageName)) {
                subtitleView.setText(subtitleView.getText() + "(" + getString(R.string.common_app_in_use) + ")");
            }
        }

        RadioButton radioButton = webViewLayout.findViewWithTag("choose");
        boolean enable = !versionName.equals(getString(NOT_INSTALL)) &&
                !versionName.equals(getString(UNSUPPORTED)) &&
                !versionName.equals(getString(NOT_ENABLED));
        ...
        try {
            ...
            if (packageInfo.applicationInfo != null) {
                ...
                if (hasWebViewLibrary) {
                    ...
                    return getString(UNSUPPORTED);
                }
            }
            return packageInfo.versionName;
        } catch (PackageManager.NameNotFoundException e) {
            return getString(NOT_INSTALL);
        }
    }
